...
export default function WorkerProfile() {
  ...
  // ✅ Get worker's own set price for a service — never use hardcoded categoryData price
  const getWorkerPrice = (serviceId) => {
    if (portfolio?.workTypePrices?.[serviceId]) {
      return Number(portfolio.workTypePrices[serviceId]);
    }
    // fallback only if worker hasn't set their own price
    const catData = CATEGORY_DATA[portfolio?.category] || null;
    const wt = catData?.workTypes?.find((w) => w.id === serviceId);
    return wt?.minPrice || 0;
  };

  // ✅ Calculate total using worker's own prices
  const calcTotal = (services) => {
    let total = 0;
    services.forEach((s) => {
      const qty = quantities[s.id] || 0;
      if (qty > 0) total += getWorkerPrice(s.id) * qty;
    });
    return total;
  };
    ...
    return services
      ...
      .map(
        (s) =>
          ...
          quantities[s.id] +
          " = ₹" +
          (getWorkerPrice(s.id) * quantities[s.id]).toLocaleString(),
      )
      .join("\n");
  };
  ...
  return (
    <div style={{ maxWidth: 640, fontFamily: "'Manrope',sans-serif" }}>
      ...
      {/* ── Profile header — NO description shown ── */}
      <div style={card}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div
            style={{
        ...
        {/* ✅ Description removed entirely — never shown */}
      </div>

      {/* ── Services — shows worker's OWN price, not categoryData range ── */}
      {selectedServices.length > 0 && (
        <div style={card}>
          <div
            style={{
          ...
          {selectedServices.map((s) => {
            const workerPrice = getWorkerPrice(s.id);
            return (
              <div
                key={s.id}
                style={{
      ...
      {view === "overview" ? (
        ...
        <div style={{ ...card, border: "1px solid rgba(200,241,53,0.2)" }}>
          ...
          {/* ✅ Service selector with quantity stepper using worker's own prices */}
          {selectedServices.length > 0 && (
            <div style={{ marginBottom: 18 }}>
              <label style={lb}>Select Services &amp; Quantity</label>
              <div
                  ...
                  >
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          ₹{workerPrice.toLocaleString()} per item
                          {qty > 0 && (
                            <span
                              style={{
                                color: "#c8f135",
                            ...
                            >
                              = ₹{(workerPrice * qty).toLocaleString()}
                            </span>
                          )}
                        </div>
                      </div>
