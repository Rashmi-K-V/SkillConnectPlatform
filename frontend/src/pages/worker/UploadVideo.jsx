...
function isUselessDescription(text) {
  if (!text) return true;
  const bad = [
    "a person", "a man", "a woman", "someone is", "person is",
    "man is", "woman is", "using a", "holding a", "standing near",
  ];
  return bad.some((b) => text.toLowerCase().includes(b));
}
...
export default function UploadVideo() {
  ...
  // Derive the worker's category from user or portfolio
  ...
  const handleUpload = async () => {
    ...
    const data = new FormData();
    data.append("video", file);
    if (workerCategory) data.append("worker_category", workerCategory);
  ...
  const handleSave = async () => {
    ...
    try {
      const category =
        workerCategory === "tailor" ? "steam_ironing" : workerCategory || "";
  ...
  const categoryLabel = getCategoryLabel(workerCategory);
  ...
  return (
    <div style={{ maxWidth: 520 }}>
      ...
      <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", marginBottom: 24 }}>
        Upload a video of your work. Skills will be auto-detected and your portfolio updated automatically.
      </p>
      ...
      {step === "idle" && (
        <div style={card}>
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.5"
            ...
            {file ? (
              <div style={{ color: "#c8f135", fontSize: 14, fontWeight: 600 }}>{file.name}</div>
            ) : (
              <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 14 }}>
                Click to select a video (mp4, mov, avi)
              </div>
      ...
      {/* ── UPLOADING — spinner only, no step list ── */}
      {step === "uploading" && (
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <svg viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5" style={{ width: 18, height: 18 }}>
                <polyline points="20 6 9 17 4 12" />
              </svg>
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#4ade80", marginBottom: 2 }}>
                Video analysed!
              </div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>
                Review the detected skills below, edit if needed, then save to your portfolio.
              </div>
            </div>
          </div>
          ...
          <div style={{ ...card, borderColor: "rgba(200,241,53,0.12)" }}>
            <label style={lb}>
              Your Category
              <span style={{
                color: "rgba(200,241,53,0.6)", fontWeight: 400,
                marginLeft: 8, fontSize: 10, textTransform: "none",
              }}>
                (set during registration)
              </span>
            </label>
            {categoryLabel ? (
              <div style={{
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: "#c8f135" }}>
                  {categoryLabel}
                </span>
              </div>
            ) : (
              <div style={{
                fontSize: 13, color: "rgba(255,165,0,0.8)",
                background: "rgba(255,165,0,0.08)",
                border: "1px solid rgba(255,165,0,0.2)",
                borderRadius: 10, padding: "10px 14px",
              }}>
                ⚠️ Category not set. Please update your profile to set your category (e.g. Electrician, Plumber, Cook, Cleaner, Steam Ironing) before saving.
              </div>
            )}
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
              ...
              {extractedSkills.map((sk) => (
                <span key={sk} style={{
                  ...
                  {sk}
                  <span onClick={() => removeSkill(sk)} style={{ cursor: "pointer", fontSize: 14, opacity: 0.6 }}>×</span>
                </span>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            onClick={handleSave}
            disabled={saving || !categoryLabel}
            style={{
              width: "100%",
              padding: "14px",
              background: saving || !categoryLabel ? "rgba(200,241,53,0.4)" : "#c8f135",
              color: "#0d0d0d",
              border: "none",
              borderRadius: 10,
              fontSize: 14,
              fontWeight: 700,
              cursor: saving || !categoryLabel ? "not-allowed" : "pointer",
              fontFamily: "'Manrope',sans-serif",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
          ...
          <button
            onClick={() => { setStep("idle"); setFile(null); }}
            style={{
              width: "100%",
              padding: "11px",
              marginTop: 10,
